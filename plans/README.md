...
- 008, 009, and 010 are independent. Execute 008 first because it touches the core config loader and establishes the review-config error-handling convention.
- 009 and 010 can run in either order after 008.
...
- Rule-walker directory-name heuristic: not worth doing now because the failure mode is a graceful skip and the contrived `*.md` directory name is not a likely production pattern.
- `event as any` in `src/index.ts`: lower leverage than the cooldown bug; no direct exploit path was established.
