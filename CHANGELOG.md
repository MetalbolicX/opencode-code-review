...
- **`ocr update` is now an unconditional clean reinstall** — aligned with `opencode-agent-skills-md` installer behaviour. There is no version gate, no registry consult, and no staleness check. Every run purges cached plugin paths and force-reinstalls from npm. Users on 1.0.4 receive the fix automatically on next `ocr update`.
